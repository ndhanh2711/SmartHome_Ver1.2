...
ERA_WRITE(V14) {
    uint8_t led1 = param.getInt();
    // Nếu led1 là 1: di chuyển servo lên 90 độ, nếu 0: di chuyển xuống 0 độ
    targetServoPos = (led1 ? 90 : 0);
    servoMoving = true;  // Đánh dấu rằng servo cần chuyển động
    ERa.virtualWrite(V14, led1); // Đồng bộ trạng thái về app
}

// Hàm cập nhật vị trí servo dạng non-blocking
void updateServoNonBlocking() {
    if (servoMoving) {
        unsigned long currentMillis = millis();
        // Kiểm tra xem đã đủ thời gian cho bước cập nhật chưa
        if (currentMillis - servoLastUpdateTime >= servoInterval) {
            servoLastUpdateTime = currentMillis;
            // Nếu vị trí hiện tại nhỏ hơn mục tiêu: tăng dần
            if (currentServoPos < targetServoPos) {
                currentServoPos++;
                servo1.write(currentServoPos);
            }
            // Nếu vị trí hiện tại lớn hơn mục tiêu: giảm dần
            else if (currentServoPos > targetServoPos) {
                currentServoPos--;
                servo1.write(currentServoPos);
            }
            // Nếu đã đạt đến vị trí mục tiêu: dừng chuyển động
            if (currentServoPos == targetServoPos) {
                servoMoving = false;
            }
        }
